...
const PATH_PARAMS = [] as Array<{ name: string; token: string }>;
const QUERY_PARAMS = [
  {
    "name": "status",
    "required": true
...
export async function handleFindPetsByStatus(
  input: Record<string, unknown>,
  context: ToolHandlerContext,
): Promise<unknown> {
  let resolvedPath = "/pet/findByStatus";
  ...
  try {
    ...
    throw new Error(
      `Network request failed for GET ${url.toString()}: ${message}`,
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `API request failed for GET ${url.toString()} (${response.status} ${response.statusText}): ${bodyText}`,
    );
  }
