...
export async function handleUpdatePet(
  input: Record<string, unknown>,
  context: ToolHandlerContext,
): Promise<unknown> {
  let resolvedPath = "/pet";
  ...
  try {
    ...
    throw new Error(
      `Network request failed for PUT ${url.toString()}: ${message}`,
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `API request failed for PUT ${url.toString()} (${response.status} ${response.statusText}): ${bodyText}`,
    );
  }
